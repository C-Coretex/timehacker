...
public class DynamicTasksController(IDynamicTaskAppService dynamicTaskAppService) : ControllerBase
{
    ...
    [HttpGet("{id:guid}")]
    public async Task<Ok<DynamicTaskReturnModel>> GetById(Guid id, CancellationToken cancellationToken = default)
    {
        var entity = await dynamicTaskAppService.GetByIdAsync(id, cancellationToken);
        NotFoundException.ThrowIfNull(entity, "DynamicTask", id.ToString());
