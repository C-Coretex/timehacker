import { Form, Input, Button, Typography, Alert, Tabs, Checkbox, message } from 'antd';
...
const LoginPage: FC = () => {
    ...
    const handleLogin = async (values: { email: string; password: string }) => {
        ...
        try {
            ...
            login({ name: '', phoneNumberForNotifications: '', emailForNotifications: values.email });
    ...
    const loginTab: TabsProps['items'][number] = {
        ...
        label: 'Login',
        children: (
            <Form form={loginForm} layout="vertical" onFinish={handleLogin} style={{ marginTop: '1rem' }}>
                <EmailField />
                <PasswordField />
                <Form.Item>
                ...
                <Form.Item>
                    <Button type="dashed" icon={<ThunderboltOutlined />} onClick={handleDevLogin} block>
                        Quick Dev Login
                    </Button>
                </Form.Item>
            </Form>
    ...
    const registerTab: TabsProps['items'][number] = {
        ...
        label: 'Register',
        children: (
            <Form form={registerForm} layout="vertical" onFinish={handleRegister} style={{ marginTop: '1rem' }}>
                <EmailField />
                <PasswordField />
                ...
                <Form.Item
                    ...
                    rules={[
                        {
                            validator: (_, value) => (value ? Promise.resolve() : Promise.reject(new Error('You must accept terms'))),
                        },
                    ]}
    ...
    return (
        <div style={{ maxWidth: 480, margin: '2rem auto' }}>
            ...
            {error && <Alert type="error" message={error} style={{ marginTop: '1rem' }} />}
